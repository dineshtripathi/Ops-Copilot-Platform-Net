...
/// <summary>
/// Manages a single OpsCopilot.McpHost child process for the SafeActions module.
///
/// Shared by <see cref="McpArmResourceReader"/> and <see cref="McpBackedLogsReader"/>
/// so both readers use a single child process rather than spawning duplicates.
///
...
///
/// Boundary rule: this class MUST NOT reference Azure.ResourceManager or
/// Azure.Monitor.Query. All Azure SDK calls happen inside McpHost.
/// </summary>
internal sealed class SafeActionsMcpHostClient : IAsyncDisposable
{
    private readonly McpHostOptions _options;
    private readonly ILogger<SafeActionsMcpHostClient> _logger;

    private readonly SemaphoreSlim _initLock = new(1, 1);
    private SdkMcpClient? _mcpClient;
    private bool _disposed;

    public SafeActionsMcpHostClient(
        McpHostOptions                      options,
        ILogger<SafeActionsMcpHostClient>   logger)
    {
        _options = options;
        _logger  = logger;
    }
        ...
        CancellationToken           ct)
    {
        ...
        {
            _logger.LogWarning(
                "SafeActions McpHost tool {Tool} timed out after {Seconds}s.",
        ...
        {
            _logger.LogError(ex, "SafeActions McpHost tool {Tool} failed.", toolName);
            await InvalidateClientAsync();
    ...
    private async Task<SdkMcpClient> GetOrCreateClientAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("SafeActions McpHost child process started and connected.");
            return _mcpClient;
        }
        finally
        {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            try   { await _mcpClient.DisposeAsync(); }
            catch (Exception ex) { _logger.LogWarning(ex, "Error disposing SafeActions McpHost child process."); }
            _mcpClient = null;
        }
